...
|----|----------|-----------|---|
| A01-1 | Subir custa {corte_max} pontos: {corte_mediana} teria ficado de fora em {anos_falha_63} das {temporadas} temporadas | indício | {temporadas} temporadas fechadas (2022–2025); as quatro anteriores (2018–2021) conferidas na base |
| A01-2 | O acesso se decide por um ponto — e quem desempata é a vitória, não o saldo | indício | {temporadas} temporadas fechadas (2022–2025); as quatro anteriores (2018–2021) conferidas na base |
| A01-3 | {sobe_fronteira} dos {sobe_de} que subiram terminaram colados na linha — por isso toda comparação roda duas vezes | indício | {fronteira_de} clube-temporadas fechadas (2022–2025), das quais {sobe_de} promovidos e {trave_de} na Trave |
| A02-1 | Quem sobe finaliza de mais perto, e isso vem antes dos pontos | firme | {n_sobe} promovidos contra {n_meio} do meio, e {n_sobe_sf} contra {n_meio_sf} sem os times colados na linha; {n_turnos} clube-temporadas no teste de anterioridade |
| A02-2 | Quem sobe não sofre menos finalização, sofre finalização pior | provável | {n_sobe} promovidos contra {n_meio} do meio, e {n_sobe_sf} contra {n_meio_sf} sem os times colados na linha; {n_turnos} clube-temporadas no teste de anterioridade |
| A02-3 | Quem sobe faz mais gol do que a chance pedia — e essa sobra não se compra | provável | {n_sobe} promovidos contra {n_meio} do meio, e {n_sobe_sf} contra {n_meio_sf} sem os times colados na linha; {n_sobe} contra {n_trave} no recorte de 5º a 8º (e {n_sobe_sf} contra {n_trave_sf} sem os colados na linha); {n_turnos} clube-temporadas na comparação entre as duas metades do ano |
| A03-1 | Depender de casa não separa quem sobe de quem fica no meio | indício | 80 clube-temporadas com todos os times ({n_sobe_com} que subiram, {n_meio_com} do meio — {n_trave_com} deles na trave — e {n_cai_com} que caíram); 52 sem os times colados na linha ({n_sobe_sem}, {n_meio_sf} com {n_trave_sem} na trave, e {n_cai_sf}) |
| A03-2 | Quem sobe ganha mais dividida dentro e fora, mas só sofre menos perigo em casa | provável | {n_sobe_com} promovidos contra {n_meio_com} do meio, e {n_sobe_sem} contra {n_meio_sf} sem os times colados na linha |
| A03-3 | Quem cai cria menos em casa e sofre mais fora | provável | {n_cai_com} do Cai contra {n_meio_com} do Meio, e {n_cai_sf} contra {n_meio_sf} sem os times colados na linha |
| A04-1 | Bola parada não é o que faz subir: nada do trabalho separa quem sobe do meio, e o que separa é gol — que já é o placar | indício | {n_sobe_com} promovidos contra {n_meio_com} do meio; sem os times colados na linha, {n_sobe_sf} contra {n_meio_sf}. A comparação com a trave (5º–8º) fica sem resposta nesta parte: são {n_sobe_com} contra {n_trave} com todos os times e {n_sobe_sf} contra 7 sem os colados na linha, um desenho que o próprio método da casa (resultados/_metodo_fronteira.md) declara enviesado entre faixas vizinhas. |
| A04-2 | Um em cada três gols da Série B sai de bola parada — e não achamos faixa que dependa mais dela | indício | {n_total} clube-temporadas, todas de 38 jogos: {n_sobe_com} que sobem, {n_meio_com} do meio (dos quais {n_trave} são a trave) e {n_cai_com} que caem |
| A04-3 | Quem cai perde no alto: o duelo aéreo é a única coisa desta parte que separa quem desce | provável | {n_cai_com} rebaixados contra {n_meio_com} do meio; sem os times colados na linha, {n_cai_sf} contra {n_meio_sf} |
| A05-1 | Nenhum jeito de construir separa quem sobe — nem ter a bola, que só aparece quando se tiram os times colados na linha | provável | {n_sobe} promovidos contra {n_meio} do meio ({sobe_folga} contra 32 sem os times a até {pts_raspando} pontos da linha); a conta do primeiro turno usa os {n_total} clube-temporadas de 2022–2025 |
| A05-2 | Dá para subir com menos da metade da bola — mas quem fez isso subiu raspando | indício | {n_sobe} promovidos de 2022 a 2025: {n_pouca_bola} com menos da metade da bola (todos a {pts_raspando_real} ponto ou menos do 5º) e {sobe_folga} que subiram com folga |
| A06-1 | Quem sobe ganha mais a dividida no chão — sobre pressão alta o estudo não decide | provável | {n_sobe_com} promovidos contra {n_meio_com} do meio, com todos os times; {n_sobe_sf} contra {n_meio_sf} sem os colados na linha |
| A06-2 | Entre subir e parar entre 5º e 8º, o jogo sem bola não mostra degrau — a dividida no chão só separa quando se tiram os times colados na linha | indício | {n_sobe_sf} promovidos contra {n_trave_sf} da trave, sem os colados na linha; {n_sobe_com} contra {n_trave} com todos os times |
| A06-3 | Quem sobe sofre tanto chute quanto o meio — o que muda é o tamanho da chance que ele entrega | provável | {n_sobe_com} promovidos contra {n_meio_com} do meio, com todos os times; {n_sobe_sf} contra {n_meio_sf} sem os colados na linha |
| A07-1 | Corrida não separou quem sobe do meio — e só uma diferença enorme apareceria neste estudo | indício | {n_sobe} que subiram contra {n_meio} do meio ({n_sobe_sf} contra {n_meio_sf} sem os times de fronteira); contra a Trave, {n_trave} contra {n_trave} e {n_sobe_sf} contra {n_trave_sf}; {n_sobe_cob} contra {n_meio_cob} quando saem os {n_cob_baixa} clube-temporada com menos jogo rastreado. As {n_total} linhas são {clubes} clubes. |
| A07-2 | Quem cai sprinta menos nos minutos sem a bola, e não é só por rodar mais o elenco | provável | {n_cai} rebaixados contra {n_meio} do meio ({n_cai_sf} contra {n_meio_sf} sem os times de fronteira; {n_cai_cob} contra {n_meio_cob} quando saem os {n_cob_baixa} clube-temporada com menos jogo rastreado). |
| A10-1 | A perna não decidiu o acesso: no returno quem subiu perdeu os mesmos {queda_sobe_mediana} metros por jogo que o meio | indício | {n_jog_sobe} jogadores de quem subiu contra {n_jog_meio} do meio ({n_sobe} clubes contra {n_meio}), só 2025 |
...
| A10-3 | Quem caiu em 2025 corre menos forte que o meio desde o 1º turno — não é cansaço de returno, é elenco | provável | {n_jog_cai} jogadores de quem caiu contra {n_jog_meio_ret} do meio no returno ({n_cai} clubes contra {n_meio}) e {n_jog_cai_turno} contra {n_jog_meio_turno} no 1º turno; só 2025 |
| A11-1 | Não é correr: é correr para onde | provável | {n} clube-temporadas (40 clubes, 2022–2025); {n_sf} sem os times colados nas linhas e {n_cob} só com cobertura física de 0,75 para cima, onde a relação fica mais forte |
| A11-2 | Correr sem a bola sobe a linha de pressão — em bola recuperada não aparece | provável | {n} clube-temporadas (40 clubes, 2022–2025); {n_sf} sem os times colados nas linhas, onde a pressão fica igual ou mais forte e as recuperações seguem sem aparecer |
| A11-3 | O físico não separa quem sobe — nem entre times de nível técnico parecido | indício | {n_estrat} das {n} clube-temporadas na estratificação: {n_sobe_media} promovidos contra {n_meio_media} do meio na faixa técnica média e {n_sobe_alta} contra {n_meio_alta} na alta, e a faixa técnica baixa não teve promovido nenhum em 2022–2025; sem os times colados nas linhas sobra a faixa alta, {sf_alta}. Os terços de sprint são {n_terco} contra {n_terco}, dentro das mesmas {n} linhas (40 clubes). |
| A12-1 | O que o clube escolhe e separa quem sobe é a chance: chutar de perto e obrigar o adversário a chutar mal — o resto é dinheiro e efeito de estar ganhando | provável | {n_sobe} promovidos contra {n_meio} do meio, e {n_sobe_sem} contra {n_meio_sem} sem os times colados na linha; na leitura de quem cai, {cai_liga} rebaixados contra {n_meio}, e {n_cai_sem} contra {n_meio_sem} |
| A12-2 | O jeito de jogar dos que subiram sem dinheiro não é atalho: quem jogou assim caiu mais do que subiu | indício | {cabem} times na faixa entre 2022 e 2025, {n_barato} casos de origem, e os 20 times de 2026 na {rodada}ª rodada de {total_rodadas} |
| A12-3 | O perfil do time barato, do jeito que está escrito, deixa de fora dois dos quatro times de que foi tirado | indício | {n_barato} casos, de 2023 a 2025 |
| A13-1 | Quem cai chega à metade do campeonato {dif_1t_cai_meio} pontos atrás do meio da tabela | indício | {n} clube-temporadas de 2022 a 2025 ({n_sem_fronteira} sem os times colados nas linhas de corte); {cai_total} rebaixados (12 no corte) e {meio_total} do meio (32 no corte) |
| A13-2 | Na metade, a tabela já tem {sobe_g4_19} dos {sobe_total} acessos e {cai_z4_19} dos {cai_total} rebaixamentos — o resto virou depois | indício | {n} clube-temporadas de 2022 a 2025 ({n_sem_fronteira} sem os times colados nas linhas de corte); {sobe_total} acessos e {cai_total} rebaixamentos |
| A13-3 | A tabela da metade dá vantagem, não garante a segunda metade | provável | {n} clube-temporadas de 2022 a 2025 ({n_sem_fronteira} sem os times colados nas linhas de corte); 27 no terço de cima e 28 no terço de baixo do 1º turno |
| A14-1 | A régua do Bloco A cabe em {n_ind} indicadores, e quem subiu aparece no alto dela | provável | {n_base} clube-temporadas: {n_sobe} que subiram, {n_meio} do meio e {n_cai} que caíram; sem os times colados na linha, {n_sobe_sf} contra {n_meio_sf} |
| A14-2 | Em duas das quatro temporadas não dá para saber se a régua vale — e quais duas muda conforme quem entra na conta | indício | 4 temporadas fechadas, 4 promovidos contra 12 do meio em cada; sem os times colados na linha, {ano_forte1} fica 4 contra 9 e {ano_fraco2} fica 2 contra 8, e {ano_fraco1} e {ano_forte2} ficam com 1 promovido, sem comparação |
| A14-3 | Em 2026 a régua põe no topo os dois primeiros da tabela — coisa que só aconteceu em {backtest_conj} das quatro temporadas fechadas | indício | {n26} times de 2026 na rodada {rodada26}, com 11 rodadas por jogar; nas temporadas fechadas, {pp_n12} times-temporada em 1º–2º contra {pp_n36} em 3º–6º — e, sem os times colados na linha, 6 contra 4 |
| J01-1 | Com um corte único de minutagem, sobra goleiro e falta atacante: passam {gk_60}% dos goleiros e {atk_60}% dos atacantes | indício | {n} jogador-temporadas (2022-2025) |
| J01-2 | Minutagem baixa não é enfermaria: a ficha de lesão não distingue quem jogou pouco de quem jogou muito | indício | {les_base} de minutagem baixa e {les_alta_base} de minutagem alta, com ponte ao Transfermarkt (2022-2025) |
| J01-3 | Time que cai roda o elenco inteiro: {cai_usados} jogadores na temporada contra {meio_usados} do meio da tabela | provável | {n_ct} clubes-temporada (2022-2025): {n_sobe} que subiram, {n_meio} do meio e {n_cai} que caíram; sem os times de fronteira, 8, 32 e 12 |
| J02-1 | {contr_geral}% dos minutos da Série B são de jogador que chegou ao clube naquele mesmo ano | indício | {n} de {n_esperado} clube-temporadas, 2022–2025 |
| J02-2 | Manter a base do ano anterior não aparece como vantagem de quem sobe | indício | {n_sobe} que subiram contra {n_meio} do meio; sem os times de fronteira, {n_sobe_sf} contra {n_meio_sf} |
| J02-3 | Quem sobe concentra {s11_s}% dos minutos nos onze mais usados; quem cai espalha o ano por {at_c} jogadores | provável | {n_sobe} que subiram e {n_cai} rebaixados contra {n_meio} do meio; sem os times de fronteira, {n_sobe_sf} e {n_cai_sf} contra {n_meio_sf} |
| J03-1 | Procuramos o titular que separa quem sobe, posição por posição, e não achamos nenhum | indício | {n_usado} titulares — {n_sobe_tot} de quem sobe contra {n_meio_tot} do meio — em {setores} setores, 2022–2025; sem os times colados na linha de acesso, 456 titulares (91 contra 365) |
| J03-2 | O duelo defensivo de quem sobe aparece na conta do time, mas não aponta nenhuma posição para contratar | indício | no time, {n_a06_s} temporadas de quem sobe contra {n_a06_m} do meio (8 contra 32 sem os times colados na linha); no jogador, {n_usado} titulares em {setores} setores (456 sem esses times) |
| J03-3 | O goleiro de quem sobe ganha a mesma bola alta que o do meio — o destaque era da porcentagem, não do goleiro | indício | {gk_n_s} goleiros de quem sobe contra {gk_n_m} do meio no recorte cheio; {gk_n_s_sem} contra {gk_n_m_sem} sem os times colados na linha de acesso |
...
| T02-1 | No G4 o que pesa é o preço do elenco | provável | {ct_n} clubes-temporada de 2022 a 2025 ({ct_top5_n} com elenco entre os 5 mais caros, {ct_meio_n} do 6º ao 10º, {ct_baixo_n} do 11º para baixo); 2026 fica à parte, como teste |
| T02-2 | No pior ano de Eduardo Baptista o time ainda passou mais de um terço das rodadas no G4 — é o piso mais alto de quem trocou de clube | indício | {eb_passagens} passagens, {eb_clubes} clubes, {eb_rodadas} rodadas (2022–2025); a passagem no Criciúma de 2026 fica à parte — 26 rodadas de temporada em curso, com Vila Nova 0×2 Criciúma faltando na base |
| T02-3 | O tempo no G4 no clube anterior não anuncia o do clube seguinte | indício | {multi} treinadores com dois clubes ou mais (2022–2025); 2026 fica à parte, como teste |
| T03-1 | Do treinador só viaja de onde o time chuta; o resto não fica com ninguém | provável | {multi} treinadores em dois clubes ou mais, dentro de {n_pass} passagens de 10 rodadas ou mais ({n_tec} treinadores), 2022–2025; {n_um_par} deles entram com um único par de clubes, e são 102 os pares entre clubes distintos. Com a temporada em andamento dentro seriam 34 em 151. A base de comparação do acaso são as {n_pass} passagens inteiras, e não só as dos que trabalharam em mais de um clube. |
| T03-2 | Trocar treinador não compra jeito de jogar novo, e os pontos que vêm depois são o time voltando ao normal | indício | {ad_n} trocas em {ad_ct} clube-temporadas ({ad_cl} clubes), 2022–2025 — as janelas se sobrepõem, porque o “depois” de uma troca vira o “antes” da seguinte, e 14 clube-temporadas entram com duas ou três trocas. Com a temporada em andamento dentro seriam 73. O placebo são {placebo_n} passagens de um treinador só, cortadas ao meio com 8 jogos ou mais de cada lado. |
| T04-1 | O nome que a lista apontava nunca subiu — e os dois que subiram com dois clubes diferentes nem entraram nela | indício | {n_lista} treinadores com 30 rodadas ou mais, em passagens de 10 rodadas ou mais e não interinas, nas quatro temporadas fechadas (2022–2025); Eduardo Baptista com {eb_pass} passagens, {eb_cl} clubes e {eb_rod} rodadas. 2026 entra só como teste. |
| T04-2 | O histórico do treinador não reaparece no clube seguinte — e esta base não conseguiria ver se reaparecesse | indício | 126 passagens de 64 treinadores nas temporadas fechadas, {t02_multi_fechadas} deles em dois clubes ou mais, e {ad_n_fechadas} trocas no meio da temporada (com 2026: {t03_pass} passagens, {t03_tec} treinadores, {t02_multi} multiclube e {ad_n} trocas). |
