...
DOC = open(DIR + "CONCLUSOES.md", encoding="utf-8").read()
SPEC = json.load(open(DIR + "conclusoes_spec.json", encoding="utf-8"))
ORDEM = json.load(open(SESSAO + "ordem_gerador_bloco2.json", encoding="utf-8"))
DECL = json.load(open(SESSAO + "declaracoes_novas.json", encoding="utf-8"))
...
CAMPO_PROIBIDO = re.compile(r"p_liq|liq2|etapa_6\.rho\.|cobertura_do_valor|p_valor_descontada|origem\\.dinheiro\\.p_permutacao|razao_verossimilhanca|trocam_no_dinheiro|desconto_dinheiro|residualizado|q_das_sobreviventes|menor_p_liq|p_liquido|p_aproveitamento_descontado_valor")
...
for c in SPEC["conclusoes"]:
    ...
    for campo, txt in list(m.items()) + [("universo", c["universo"])]:
        ...
        texto_ok = re.sub(r"não é folha nem gasto|não é quanto se gastou|não é custo|o estudo não separa as duas coisas|o estudo não separa a origem do valor do elenco", "", render(txt or "", val))
        for p in PROIBIDAS:
...
# ------------------------------------------------------------ 2c. conserto de 15/09: rodapés, linhas de faixa, "no limite", teto e p por clube
# a varredura é um módulo à parte (varredura_rodapes), sem nada do montador: dinheiro descontado, repetição no clube e
# semente de 14/09 só passam num pedaço marcado como registro; "no limite" só no selo da zona cinzenta
import varredura_rodapes as VR
pos_id = {c["id"]: c for c in SPEC["conclusoes"]}
varridos = 0
for c in SPEC["conclusoes"]:
    val = c["valores_14_09"]
    textos = {"rodape": c["documento"]["rodape"], "faixa": c["vale_pela_faixa_de_pontos"]["texto_documento"] or "", "universo": render(c["universo"], val)}
    textos.update({"molde." + k: render(v, val) for k, v in c["molde"].items() if isinstance(v, str)})
    for nome, p, pd in VR.achados(c, textos):
        F(f"{c['id']}: {nome} ainda tem '{p}' fora de registro: {pd[:140]}")
    varridos += 1
# o documento, bloco a bloco (o que o leitor vê), com a mesma varredura
for i, b in doc_blocos.items():
    if i in pos_id:
        for nome, p, pd in VR.achados(pos_id[i], {"documento": "\n".join(b["linhas"])}):
            F(f"{i}: bloco do documento ainda tem '{p}' fora de registro: {pd[:140]}")
# as cinco, linha a linha
sec5 = DOC[DOC.index("## AS CINCO QUE VOCÊ PRECISA LER"):]
sec5 = sec5[:sec5.index("\n---")]
for ln in re.findall(r"^\d\. .*$", sec5, re.M):
    mm = re.match(r"^\d\. \*\*\[[^\]]+\] (\S+) · ", ln)
    if mm and mm.group(1) in pos_id:
        for nome, p, pd in VR.achados(pos_id[mm.group(1)], {"cinco": ln}):
            F(f"cinco ({mm.group(1)}): '{p}' fora de registro: {pd[:140]}")
# comparação declarada que o próprio critério diz ter sido escolhida depois de olhar: a máquina tem de tratá-la como escolhida depois
for c in SPEC["conclusoes"]:
    for k in c["criterios_do_selo"]:
        if k.get("papel") == "declarada" and k.get("passou") is True and re.search(r"depois de olhar", k["criterio"] + " " + str(k["valor_14_09"])):
            F(f"{c['id']} {k['id']}: critério 'declarada' passa, mas diz que a comparação foi escolhida depois de olhar (sem teto)")
m8 = pos_id.get("M8")
if not m8 or (m8.get("entradas_da_regua") or {}).get("declarada_constante") is not False or m8["comparacao_declarada_antes"] is not False:
    F("M8: a divisão do título foi escolhida depois de olhar; a máquina tem de ler declarada = false (teto moderado)")
# no por clube o p que decide o selo é o bruto: p_principal = menor p_clube do setor, lido sem arredondar
for c in SPEC["conclusoes"]:
    kp = [k for k in c["criterios_do_selo"] if k.get("papel") == "p" and str(k.get("caminho")).endswith(".p5_clube")]
    if not kp:
        continue
    base = kp[0]["caminho"][:-len(".p5_clube")]
    ok, itens = resolver(base + ".itens")
    esperado = min(x["p_clube"] for x in itens if x.get("p_clube") is not None) if ok else None
    pp = c["p_principal"]
    if (pp.get("caminho"), pp.get("subcampo"), pp.get("agregacao"), pp.get("valor_14_09")) != (base + ".itens", "p_clube", "min", esperado):
        F(f"{c['id']}: p_principal por clube tem de ser o menor p_clube de {base}.itens ({esperado}); está {pp}")
# DIN-01 e DIN-02: o critério das conferências por ano escrito na régua, com a pergunta em aberto para o dono
if "acima de 0,5 em todo ano" not in DOC or "Pergunta em\n   aberto para o dono:" not in DOC or "conferencia_por_ano" not in SPEC["regua"]:
    F("régua: o critério das conferências por ano (DIN-01, DIN-02) tem de estar escrito no documento e no spec, com a pergunta em aberto para o dono")

# ------------------------------------------------------------ 3. contagem e cinco
cont ={s: sum(1 for c in SPEC["conclusoes"] if c["forca_esperada_hoje"] == s) for s in ORD}
tot = len(SPEC["conclusoes"])
ref = SPEC["contagem_de_selos_referencia_15_09"]
...
for cp in campos:
    ...
    if "[]" in cp["caminho"].replace("linhas[]", "").replace("metricas[]", "").replace("setores[]", "").replace("quartis[]", "").replace("itens[]", "").replace("por_ano[]", "").replace("reguas[]", "") :
    ...
    for ph in re.findall(r"<[A-Za-z_]+>", cp["caminho"]):
        if ph not in (cp.get("valores_permitidos") or {}):
            AVISOS.append(f"ordem (arquivo do dono das declarações): campo {cp['caminho']} sem valores_permitidos para {ph}")
...
if AVISOS:
    print(f"\nAVISOS ({len(AVISOS)}), não travam a prova, são de arquivo de outro dono:")
    for a_ in AVISOS:
        print(" -", a_)
if falhas:
    print(f"\nFALHAS ({len(falhas)}):")
