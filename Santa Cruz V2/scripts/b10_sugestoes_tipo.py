...
no exterior em ligas mais fracas. Filtros: piso de 27 km/h, até 32 anos, sem vetados, valor ≤ € 2 MM.
...
def main():
    ...
    A = A[A.apply(lambda r: r.tipo in P.get(r.setor, []), axis=1)]
    A = A[(A.psv99 >= 27) & (A.idade <= 32)]
    A = A[[not fora(j, c) for j, c in zip(A.jogador, A.clube)]]
    A = A[~caro(A)]
    A = A.sort_values("nota", ascending=False).drop_duplicates(["chave", "clube"])
    A.to_csv(os.path.join(OUT, "sugestoes.csv"), index=False)
    ...
    md = ["# Bloco 10 — Sugestões pelo tipo físico de quem sobe", "",
    ...
    for pos in ORDEM:
        ...
        for merc in ["Série B", "Sul-americanas", "Exterior (ligas mais fracas)"]:
            ...
            for i, r in enumerate(x.itertuples(), 1):
                ...
                md.append(f"| {i} | **{r.jogador}**{'' if r.livre_2027 else ' (e)'} | {r.clube} |{liga} {int(r.idade)} | {dt(r.contrato)} | {r.tipo} | "
